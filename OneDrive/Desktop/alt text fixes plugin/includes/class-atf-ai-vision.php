...
class ATF_AI_Vision {
	...
	public static function register_settings() {
		...
		add_settings_field( 'atf_ai_enabled', esc_html__( 'Enable AI Vision fallback', 'alt-text-fixer' ), array( __CLASS__, 'field_enabled' ), 'alt-text-fixer', 'atf_ai_vision' );
		...
		add_settings_field( 'atf_ai_api_key', esc_html__( 'OpenAI API key', 'alt-text-fixer' ), array( __CLASS__, 'field_api_key' ), 'alt-text-fixer', 'atf_ai_vision' );
	}

	public static function section_description() {
		echo '<p>' . esc_html__( 'AI is used only when the normal title/file-name/context logic produces an empty or generic candidate. The key may also be defined as ATF_OPENAI_API_KEY in wp-config.php.', 'alt-text-fixer' ) . '</p>';
	}

	public static function field_enabled() {
		$settings = self::settings();
		printf( '<label><input type="checkbox" name="%1$s[enabled]" value="yes" %2$s> %3$s</label>', esc_attr( self::OPTION ), checked( 'yes', $settings['enabled'], false ), esc_html__( 'Generate descriptive alt text with AI when normal rules cannot produce a useful value.', 'alt-text-fixer' ) );
	}

	public static function field_model() {
		$settings = self::settings();
		printf( '<input type="text" class="regular-text" name="%1$s[model]" value="%2$s" placeholder="%3$s">', esc_attr( self::OPTION ), esc_attr( $settings['model'] ), esc_attr( self::DEFAULT_MODEL ) );
	}
	...
	public static function sanitize_settings( $input ) {
		...
		return array(
			...
			'model'   => ! empty( $input['model'] ) ? sanitize_text_field( $input['model'] ) : self::DEFAULT_MODEL,
	...
	public static function settings() {
		...
		return array(
			...
			'model'   => isset( $saved['model'] ) && $saved['model'] ? $saved['model'] : self::DEFAULT_MODEL,
